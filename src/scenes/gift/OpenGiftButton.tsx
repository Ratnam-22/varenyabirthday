...
export const OpenGiftButton: React.FC<OpenGiftButtonProps> = ({ className }) => {
  ...
  const handleClick = () => {
    ...
    eventBus.emit('action:open-gift');
  ...
  return (
    <div
      className={cn(
        'transition-all duration-1000 ease-out pointer-events-auto will-change-transform mt-12',
        isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4',
        hasTriggered && 'pointer-events-none opacity-0 transition-opacity duration-300',
        className
      )}
    >
      <Magnetic disabled={hasTriggered}>
        <Float disabled={hasTriggered}>
          <Button
            ...
            onClick={handleClick}
            className={cn(
              'relative group overflow-hidden px-10 py-3.5 text-sm font-light tracking-[0.3em] uppercase text-[#fdfbf7]',
              'border border-[rgba(212,175,55,0.35)] bg-[rgba(18,19,28,0.7)] backdrop-blur-md rounded-full',
              'transition-all duration-500 ease-out focus-ring',
              !hasTriggered &&
                'hover:border-[rgba(212,175,55,0.8)] hover:shadow-[0_0_35px_rgba(212,175,55,0.4)] hover:-translate-y-0.5',
              isPressed ? 'scale-95 duration-150' : 'scale-100'
            )}
            data-cursor-state={hasTriggered ? 'default' : 'interactive'}
          >
            {/* Animated Gold Shimmer Accent */}
            <span
              aria-hidden="true"
